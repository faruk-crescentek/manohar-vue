...
const admin_id = route.params.id;

const adminService = new AdminService();

const getAdmin = (admin_id) => {
    adminService.getAdmin(admin_id).then((data) => {
        admin.value = data;
    });
}
...
const updateAdmin = admin_id => {
    ...
    if (admin.value.email && admin.value.email.trim()) {
        ...
        adminService.updateAdmin(admin_id, admin.value).then((data) => {
            admin.value = data.data;
            toast.add({ severity: 'success', summary: 'Successful', detail: 'Admin Updated', life: 3000 });
        });
    }
};
</script>
        ...
        <div class="col-12">
            <div class="card">
                ...
                <div>
                    ...
                    <Button label="Submit" class="mr-2 mb-2" @click="updateAdmin(admin.id)" />
                </div>
            </div>
        </div>
    </div>
